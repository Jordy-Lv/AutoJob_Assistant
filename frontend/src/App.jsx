...
export default function App() {
  ...
  const loadAll = useCallback(async (options = {}) => {
    ...
    try {
      ...
      setSelectedJobId((current) => current || jobsData.jobs?.[0]?.id || null);
    } catch (error) {
      ...
      <main className="workspace" id="main-content">
        ...
        {loading && !overview ? (
          ...
          <ErrorBoundary key={view}>
            ...
            {view === "search" && <SearchView reload={() => loadAll({ silent: true })} showToast={showToast} setView={setView} profile={profile} />}
            {view === "documents" && (
              <DocumentsView
                documents={documents}
                jobs={jobs}
      ...
      <button className="job-card-select" onClick={onSelect} type="button">
        ...
        <div className="job-card-footer">
          <div className="job-card-badges">
            {!job.viewed && <span className="new-badge"><Sparkles size={12} /> Nueva</span>}
            {job.viewed && <span className="status-badge">Vista</span>}
  ...
  return (
    <div className="job-detail">
      ...
      <div className="detail-actions">
        ...
        {job.url ? (
          <a className={openOriginalIsPrimary ? "button primary" : "button secondary"} href={job.url} target="_blank" rel="noreferrer" onClick={() => onView?.()}>
            <ExternalLink size={16} /> Abrir oferta original
          </a>
        ) : (
          <button className="button secondary" disabled type="button">
            <ExternalLink size={16} /> Sin URL original
          </button>
        )}
        {!isAlreadyApplied && (
          <button className="button secondary" onClick={onMarkApplied} type="button">
            <CheckCircle2 size={16} /> Marcar como aplicada
          </button>
        )}
        {job.score != null && (
        ...
        <button className="button ghost danger" onClick={onDelete} type="button">
          <Trash2 size={16} /> Descartar oferta
        </button>
      </div>
      ...
      {/* Estado — inline, sin caja, con acento de color */}
      <div className="detail-status-row">
        <span className="detail-label">Estado</span>
        <select
          className="status-select-compact"
          data-status={STATUSES.includes(job.status) ? job.status : "Nueva"}
      ...
      {/* Por qué encaja — borderless */}
      <div className="detail-section">
        <div className="detail-section-title">
          <Target size={13} />
          <span className="detail-section-label">Por qué encaja</span>
        </div>
        {job.reasons?.length ? (
          ...
          <p className="detail-empty-hint">Ejecuta el analisis para ver por qué esta oferta puede ser una buena oportunidad.</p>
        )}
      </div>
      ...
      <div className="detail-section">
        <div className="detail-section-title">
          <AlertCircle size={13} />
          <span className="detail-section-label">Brechas a reforzar</span>
          {job.gaps?.length > 0 && <span className="detail-section-count">{job.gaps.length}</span>}
        </div>
        {job.gaps?.length ? (
          <>
            <p className="detail-section-caption">Habilidades detectadas como brecha para esta oferta.</p>
            <div className="gap-chips">
              {job.gaps.slice(0, 7).map((g) => <span key={g}>{g}</span>)}
            </div>
          </>
        ) : (
          <p className="detail-empty-hint">Sin brechas detectadas todavia.</p>
        )}
      </div>

      <div className="detail-divider" />

      {/* CV y carta — card interactiva */}
      <div className="detail-docs-block">
        <div className="detail-docs-header">
          <FileText size={14} />
          <span className="detail-section-label">CV y carta</span>
          {documents.length > 0 && (
            <span className="doc-count">{documents.length} {documents.length === 1 ? "archivo" : "archivos"}</span>
          )}
        </div>
        {documents.length ? (
          ...
          <div className="docs-empty-state">
            ...
            <div className="docs-empty-copy">
              ...
              <span>Genera el CV y carta cuando estes listo para aplicar a esta oferta.</span>
            </div>
            <button className="button primary docs-cta" onClick={onGenerate} type="button">
              <FileText size={15} /> Generar CV y carta
            </button>
          </div>
        )}
      </div>
      ...
      <details className="description-box">
        <summary>
          <Eye size={14} />
          <span>Ver descripcion guardada</span>
        </summary>
                ...
                <div className="saved-search-actions">
                  ...
                  {pendingDeleteId === saved.id ? (
                    <>
                      ...
                      <button className="button ghost compact" onClick={() => setPendingDeleteId(null)} type="button">
                        Cancelar
                      </button>
                    </>
                  ) : (
                    <button className="button ghost compact danger" onClick={() => setPendingDeleteId(saved.id)} type="button">
  ...
  return (
    <div className="form-grid">
      ...
      <Field
        ...
        placeholder="70"
      />
      <SelectField label="Filtro de fecha" value={form.date_filter} onChange={(v) => update("date_filter", v)} options={DATE_FILTER_OPTIONS} />
...
function AutoSearchPanel({ sources, reload, showToast, setView, profile }) {
  ...
  const availableSourceIds = useMemo(
    () => sources.filter((s) => s.enabled && s.configured).map((s) => s.id),
    [sources],
  );

  useEffect(() => {
    setSelectedSourceIds((current) => {
      const currentAvailable = current.filter((id) => availableSourceIds.includes(id));
      return currentAvailable.length ? currentAvailable : availableSourceIds;
    });
  }, [availableSourceIds]);
  ...
  async function run() {
    ...
    if (!availableSourceIds.length) {
      showToast("No hay fuentes disponibles para buscar.", "error");
      return;
    }
    const selectedAvailableIds = selectedSourceIds.filter((id) => availableSourceIds.includes(id));
    const baseSourceIds = selectedAvailableIds.length ? selectedAvailableIds : availableSourceIds;
    setRunning(true);
    try {
      const attempts = buildSearchAttempts(baseSourceIds, availableSourceIds, dateFilter);
      let result = null;
      let usedAttempt = attempts[0];

      for (const attempt of attempts) {
        result = await runSearchAttempt({
          keywords,
          location,
          remoteOnly,
          juniorOnly,
          ...
          dateFilter: attempt.dateFilter,
        });
        usedAttempt = attempt;
        if ((result.total_found || 0) > 0 || (result.total_new || 0) > 0 || (result.total_updated || 0) > 0) {
          break;
        }
      }

      setSelectedSourceIds(usedAttempt.sourceIds);
      setDateFilter(usedAttempt.dateFilter);
      setSummary(result);
      const newCount = result.total_new ?? 0;
      const updatedCount = result.total_updated ?? 0;
      const fallbackText = usedAttempt.reason ? ` ${usedAttempt.reason}` : "";
      if ((result.total_found || 0) > 0) {
        showToast(
          newCount > 0
            ? `${newCount} ofertas nuevas guardadas, ${updatedCount} actualizadas.${fallbackText}`
            : `${result.total_found} ofertas encontradas.${fallbackText} ${updatedCount} actualizadas, ${result.duplicates || 0} duplicadas.`,
        );
      } else {
        showToast("No aparecieron ofertas con esos filtros. Prueba quitar Solo junior o usar Java Developer en vez de solo Java.", "error");
      }
      await reload();
    } catch (error) {
      showToast(error.message, "error");
  ...
  return (
    <Panel icon={Globe2} title="Buscar por palabra clave" action="Metodo principal">
      <div className="form-grid">
        ...
        <SelectField label="Antigüedad máxima" value={dateFilter} onChange={setDateFilter} options={DATE_FILTER_OPTIONS} />
        <div className="field-wide field-checkboxes">
          <label className="toggle-field">
          ...
          <label className="toggle-field">
            ...
            <span>Solo junior / sin experiencia</span>
          </label>
          <label className="toggle-field">
        ...
        <div className="field-wide source-inline">
          ...
          <details className="source-settings">
            ...
            <div className="source-actions">
              <button className="button ghost compact" onClick={selectAllSources} type="button">
                Todas las fuentes
              </button>
            </div>
            <div className="source-picker">
      ...
      <div className="form-footer">
        ...
        <button className="button primary" onClick={run} disabled={running || !availableSourceIds.length} type="button">
          <Search size={16} />
...
function buildSearchAttempts(selectedIds, availableIds, dateFilter) {
  ...
  const baseSources = selected.length ? selected : available;
  attempts.push({ sourceIds: baseSources, dateFilter, reason: "" });

  if (available.length > baseSources.length) {
    attempts.push({
      sourceIds: available,
      dateFilter,
      reason: "Reintente con todas las fuentes disponibles.",
    });
  }

  const relaxedDate = relaxedFreshDateFilter(dateFilter);
  if (relaxedDate !== dateFilter) {
    attempts.push({
      sourceIds: available,
      dateFilter: relaxedDate,
      reason: "Reintente ampliando la frescura hasta 4 dias.",
    });
  }
...
function relaxedFreshDateFilter(value) {
  if (["24h", "1d", "2d", "3d"].includes(value)) return "4d";
  return value;
}
...
function dedupeSearchAttempts(attempts) {
  ...
  return attempts.filter((attempt) => {
    const key = `${attempt.dateFilter}:${attempt.sourceIds.join(",")}`;
    if (seen.has(key)) return false;
    seen.add(key);
    return attempt.sourceIds.length > 0;
  });
}
...
// ----------------------------------------------------------------------------
// Settings view — fuentes en vivo desde la API
// ----------------------------------------------------------------------------
  ...
  return (
    <div className="page-stack">
      <section className="page-heading">
        ...
        <p>Revisa la conexion, las fuentes activas y los servicios opcionales.</p>
      </section>
      ...
      <section className="settings-grid">
        <Panel icon={Database} title="Base de datos" action={overview?.health?.ok ? "conectada" : "revisar"}>
          <div className="settings-card">
            <StatusBadge
              status={overview?.health?.ok ? "Aplicada" : "Descartada"}
              label={overview?.health?.ok ? "Conectada" : "Con error"}
            />
            <strong>{overview?.health?.database_url || "URL no disponible"}</strong>
            {overview?.health?.error && <span className="source-error-line">{overview.health.error}</span>}
          </div>
        </Panel>

        <Panel icon={WandSparkles} title="Analisis de compatibilidad" action="local + IA opcional">
          <div className="settings-card">
            <StatusBadge status="Interesante" label="Disponible" />
            <strong>Heuristica local activa</strong>
            <span>
              Configura <code>OPENAI_API_KEY</code> para analisis con IA desde la ficha de cada oferta.
            </span>
          </div>
        </Panel>

        <Panel icon={Bell} title="Alertas por Telegram" action="proxima fase">
          <div className="settings-card">
            <StatusBadge status="Nueva" label="Configurable" />
            <strong>Configura TELEGRAM_BOT_TOKEN y TELEGRAM_CHAT_ID</strong>
            <span>El scheduler enviara alertas con ofertas nuevas que superen el score minimo de cada busqueda guardada.</span>
          </div>
        </Panel>
      </section>
      ...
      <Panel
        ...
        action={sourcesLoading ? "cargando..." : `${activeSources.length} activas · ${missingApiKey.length} sin configurar`}
      >
        {sourcesLoading && <p className="muted-line">Cargando fuentes...</p>}
        {!sourcesLoading && (
          <div className="source-status-list">
            ...
            {!sources.length && (
              <p className="muted-line">No se pudieron cargar las fuentes. Verifica que el backend este corriendo.</p>
            )}
          </div>
        )}
        {!sourcesLoading && missingApiKey.length > 0 && (
          <div className="settings-hint">
            <AlertCircle size={15} />
            <span>
              Las fuentes sin configurar necesitan variables de entorno en el archivo <code>.env</code>.
              Reinicia el backend tras agregarlas.
            </span>
          </div>
        )}
      </Panel>
...
function SourceStatusCard({ source }) {
  const isActive = source.configured && source.enabled;
  const isMissing = !source.configured && source.requires_api_key;
  const isDisabled = !source.enabled;

  let badgeStatus = "neutral";
  let badgeLabel = "Desconocida";
  if (isDisabled) { badgeStatus = "neutral"; badgeLabel = "Deshabilitada"; }
  else if (isActive) { badgeStatus = "Aplicada"; badgeLabel = "Activa"; }
  else if (isMissing) { badgeStatus = "Nueva"; badgeLabel = "Sin API key"; }
  else if (source.status === "unavailable") { badgeStatus = "Descartada"; badgeLabel = "Error"; }

  return (
    <article className={`source-status-card ${isActive ? "active" : "inactive"}`}>
      <div className="source-status-header">
        <div>
          <strong>{source.name}</strong>
          {source.description && <span className="source-desc">{source.description}</span>}
        </div>
        <StatusBadge status={badgeStatus} label={badgeLabel} />
      </div>
      {isMissing && source.env_vars?.length > 0 && (
        <p className="source-env-hint">
          Requiere en <code>.env</code>:{" "}
          {source.env_vars.map((v) => (
            <code key={v} className="env-var">{v}</code>
          ))}
        </p>
      )}
      {source.error && !isMissing && (
        <p className="source-error-line">{source.error}</p>
      )}
    </article>
  );
}
